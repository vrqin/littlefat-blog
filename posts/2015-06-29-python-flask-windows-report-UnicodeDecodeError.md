...
Windows 运行python flask时报错：
> UnicodeDecodeError: 'ascii' codec can't decode byte 0xc4 in position 33: ordinal not in range(128)

经确认此问题为Python2.7在Windows上一个bug。

解决方法：

* 使用官方patch： http://bugs.python.org/file19332/9291a.patch

改动代码如下:
> 1>加入from itertools import count

> 2>修改 def enum_types(mimedb)为:
```
